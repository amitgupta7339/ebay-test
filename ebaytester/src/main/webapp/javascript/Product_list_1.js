...
//GLOBAL VARIABLES
var product_list =""; //previously "r_tmp" real time product list shown on the page items
var selected_product_list = "";//currently selected subcategory/category product list
var filter_gui = "";
...
function showSubCategoryList()
{
	...
	$.ajax({
	       type: "get",
	       url:  "http://localhost:5224/ebaytester/webapi/category/" + category,
	    	   	...
	    	   	for(y in subcategory_list)
	    	   		{
	    	   			subCategory_list_gui+='<p style="font-size:12px">'
	    	   										+'<a href="#" onclick = SubCategoryFilter('+subcategory_list[y].sub_category_id+')>'+subcategory_list[y].sub_category_name+'</a>'
	    	   											+'</p>';
	    	   		}
	...
	for(y in product_list)
	   {
		call_string = "productDescription("+product_list[y].productId+")";
	   	ProductListGUI += '<li class="list-group-item">'
	   					+'<div class="row">'
	   						+'<div class = "col-sm-4">'
	   							+'<a href="#" onclick='+call_string+'>'
	   								+'<img src="'+product_list[y].productImageUrl+'" style="max-width:300px; max-height:300px" alt="product">'
	   							+'</a>'
	   						+'</div>'
	   						+'<div class = "col-sm-8">'
	   								+'<h4><a class="hover" onclick='+call_string+'>'+product_list[y].productName+'</a></h4>'
	   								+'<h5 style="color:grey">'+product_list[y].productDescription
	   									+'</h5>'
	   								+'<h4><br><strong>Rs.<strike>'+product_list[y].productPrice+'</strike> '+product_list[y].productPrice*(1-(product_list[y].productDiscount)/100)+'</strong></h4>'
	   								+'<h5 style="color:grey"><strong>Item Id : </strong> '+product_list[y].productItemId
	   									+'<button type="button" style = "float:right" class="btn btn-primary" onclick="buybutton('+product_list[y].productId+/*','+product_list[y].productName+','+product_list[y].productPrice+*/')">Buy Now</button></h5>'
	   								+'<h5 style="color:grey"><strong>Seller : </strong> '+product_list[y].sellerName+'</h5>'
	   								+'<h5 style="color:grey"><strong>Discount : </strong>'+product_list[y].productDiscount+'%'
	   									+'<button type="button" style = "float:right" class="btn btn-primary" onclick="addToCartButton('+product_list[y].productId+/*','+product_list[y].productName+','+product_list[y].productPrice+*/')">Add To Cart</button></h5>'
	   								+'<h5 style="color:grey"><strong>Shipping : </strong>'+product_list[y].productShipping+'</h5>'
	   						+'</div>'
	   					+'</div>'
	   				+'</li>';
	    };
...
function SubCategoryFilter(subCategory_id)
	{
	   ...
	   document.getElementById("filters").innerHTML = filter_gui;*/ // =>use this code to change filters according to subcategory selection
...
function showBrandFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!brand.includes(selected_product_list[y].productBrand) && selected_product_list[y].productBrand !="" && selected_product_list[y].productBrand != null)//if particular brand is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<brand.length;i++)
		   			{
		   				brandFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+brand[i]+'" onchange = "brandFilter(this)">'+brand[i]+'</label>'
		   							  +'</div>';
		   			}
		   		brandFilterGUI += "<br>";
		   }
...
function showColorFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!color.includes(selected_product_list[y].productColor) && selected_product_list[y].productColor !="" && selected_product_list[y].productColor != null)//if particular color is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<color.length;i++)
		   			{
		   				colorFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+color[i]+'" onchange = "colorFilter(this)">'+color[i]+'</label>'
		   							  +'</div>';
		   			}
		   		colorFilterGUI += "<br>";
		   }
...
function showConditionFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!condition.includes(selected_product_list[y].productCondition) && selected_product_list[y].productCondition !="" && selected_product_list[y].productCondition != null)//if particular condition is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<condition.length;i++)
		   			{
		   				conditionFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+condition[i]+'" onchange = "conditionFilter(this)">'+condition[i]+'</label>'
		   							  +'</div>';
		   			}
...
function showStorageFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!storage.includes(selected_product_list[y].productStorage) && selected_product_list[y].productStorage !="" && selected_product_list[y].productStorage != null)//if particular storage is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<storage.length;i++)
		   			{
		   				storageFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+storage[i]+'" onchange = "storageFilter(this)">'+storage[i]+'</label>'
		   							  +'</div>';
		   			}
		   		storageFilterGUI += "<br>";
		   }
...
function showOSFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!os.includes(selected_product_list[y].productOS) && selected_product_list[y].productOS !="" && selected_product_list[y].productOS != null)//if particular os is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<os.length;i++)
		   			{
		   				osFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+os[i]+'" onchange = "osFilter(this)">'+os[i]+'</label>'
		   							  +'</div>';
		   			}
		   		osFilterGUI += "<br>";
		   }
...
function showYearFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!year.includes(selected_product_list[y].productYear) && selected_product_list[y].productYear !="" && selected_product_list[y].productYear != null)//if particular year is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<year.length;i++)
		   			{
		   				yearFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+year[i]+'" onchange = "yearFilter(this)">'+year[i]+'</label>'
		   							  +'</div>';
		   			}
		   		yearFilterGUI += "<br>";
		   }
...
function showGenderFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!gender.includes(selected_product_list[y].productGender) && selected_product_list[y].productGender !="" && selected_product_list[y].productGender != null)//if particular gender is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<gender.length;i++)
		   			{
		   				genderFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+gender[i]+'" onchange = "genderFilter(this)">'+gender[i]+'</label>'
		   							  +'</div>';
		   			}
		   		genderFilterGUI += "<br>";
		   }
...
function showApplicableFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!applicable.includes(selected_product_list[y].productApplicable) && selected_product_list[y].productApplicable !="" && selected_product_list[y].productApplicable != null)//if particular applicable is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<applicable.length;i++)
		   			{
		   				applicableFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+applicable[i]+'" onchange = "applicableFilter(this)">'+applicable[i]+'</label>'
		   							  +'</div>';
		   			}
...
function showMaterialFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!material.includes(selected_product_list[y].productMaterial) && selected_product_list[y].productMaterial !="" && selected_product_list[y].productMaterial != null)//if particular material is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<material.length;i++)
		   			{
		   				materialFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+material[i]+'" onchange = "materialFilter(this)">'+material[i]+'</label>'
		   							  +'</div>';
		   			}
...
function showClothingSizeFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!clothingSize.includes(selected_product_list[y].productClothingSize) && selected_product_list[y].productClothingSize !="" && selected_product_list[y].productClothingSize != null)//if particular clothingSize is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<clothingSize.length;i++)
		   			{
		   				clothingSizeFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+clothingSize[i]+'" onchange = "clothingSizeFilter(this)">'+clothingSize[i]+'</label>'
		   							  +'</div>';
		   			}
...
function showWarrantyTypeFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!warrantyType.includes(selected_product_list[y].productWarrantyType) && selected_product_list[y].productWarrantyType !="" && selected_product_list[y].productWarrantyType != null)//if particular warrantyType is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<warrantyType.length;i++)
		   			{
		   				warrantyTypeFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+warrantyType[i]+'" onchange = "warrantyTypeFilter(this)">'+warrantyType[i]+'</label>'
		   							  +'</div>';
		   			}
...
function showWarrantyFilter(selected_product_list)
{
	...
	for(y in selected_product_list)
	   {
	    	if(!warranty.includes(selected_product_list[y].productWarranty) && selected_product_list[y].productWarranty !="" && selected_product_list[y].productWarranty != null)//if particular warranty is not there in list then add new one
	    		{
		   		...
		   		for(i=0;i<warranty.length;i++)
		   			{
		   				warrantyFilterGUI += '<div class="checkbox">'
		   								+'<label><input type="checkbox" value="'+warranty[i]+'" onchange = "warrantyFilter(this)">'+warranty[i]+'</label>'
		   							  +'</div>';
		   			}
...
function showList()
{
	...
	$.ajax({
	       type: "get",
	       url:  "http://localhost:5224/ebaytester/webapi/products/"+url_proceed,
	       success: function(response){
...
function clothingSizeFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  clothingSize_filt.splice(clothingSize_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
function materialFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  material_filt.splice(material_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
function applicableFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  applicable_filt.splice(applicable_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
function genderFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  gender_filt.splice(gender_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
function yearFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  year_filt.splice(year_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
function osFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  os_filt.splice(os_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
function storageFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  storage_filt.splice(storage_filt.indexOf($(obj).val()),1);//to delete the unchecked element
...
function warrantyTypeFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  warrantyType_filt.splice(warrantyType_filt.indexOf($(obj).val()),1);//to delete the unchecked element
...
function warrantyFilter(obj)
{
	if($(obj).is(":checked")){//When checked
		...
		$("#sortFilter").val("None");
		  ...
		  warranty_filt.splice(warranty_filt.indexOf($(obj).val()),1);//to delete the unchecked element
		  product_list = getListByFilters(selected_product_list);
		  document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		  $("#sortFilter").val("None");
	  }
...
{
	list = selected_product_list.filter(function(el) {
		    return ((starting_price != -1 && ending_price !=-1) ? (el.productPrice >= starting_price && el.productPrice <= ending_price):true) && (brand_filt.length != 0 ? brand_filt.includes(el.productBrand) : true) &&  (color_filt.length != 0 ? color_filt.includes(el.productColor) : true) && (condition_filt.length != 0 ? condition_filt.includes(el.productCondition) : true) &&  (warranty_filt.length != 0 ? warranty_filt.includes(el.productWarranty) : true) &&  (warrantyType_filt.length != 0 ? warrantyType_filt.includes(el.productWarrantyType) : true) &&  (storage_filt.length != 0 ? storage_filt.includes(el.productStorage) : true) &&  (os_filt.length != 0 ? os_filt.includes(el.productOS) : true) &&  (year_filt.length != 0 ? year_filt.includes(el.productYear) : true) &&  (gender_filt.length != 0 ? gender_filt.includes(el.productGender) : true) &&  (applicable_filt.length != 0 ? applicable_filt.includes(el.productApplicable) : true) &&  (material_filt.length != 0 ? material_filt.includes(el.productMaterial) : true) &&  (clothingSize_filt.length != 0 ? clothingSize_filt.includes(el.productClothingSize) : true);
		});
	return list;
}
...
function priceFilter(start_price,end_price)
	{
		...
		product_list = selected_product_list.filter(function(el) {
 		    return (el.productPrice >= start_price && el.productPrice <= end_price) && (brand_filt.length != 0 ? brand_filt.includes(el.productBrand) : true) &&  (color_filt.length != 0 ? color_filt.includes(el.productColor) : true) && (condition_filt.length != 0 ? condition_filt.includes(el.productCondition) : true);
 		});
		document.getElementById("product_list").innerHTML = createProductListGUI(product_list);
		$("#sortFilter").val("None");
	}
